...
def send_climb(climbinfo, name):
	...
	data['Problem.Grade'] = climbinfo['Grade']
	final_move_index = len(climbinfo['Moves'])-1
	data['Problem.Moves[{}].IsEnd'.format(final_move_index)]='true'

	for i in range(len(climbinfo['Moves'])):
		data['Problem.Moves[{}].Description'.format(i)]=climbinfo['Moves'][i]
