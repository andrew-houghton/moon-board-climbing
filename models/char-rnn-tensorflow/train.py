...
def build_model(base_save_dir):
    # This function exposes command line arguments
    # so that model training can be triggered from elsewhere in the project

    # Create a parser with default arguments
    parser = get_default_args()
    args = parser.parse_args()  # process it
...
def train(args):
    ...
    if not os.path.isdir(args.data_dir + 'save/'):
        os.makedirs(args.data_dir + 'save/')
    with open(os.path.join(args.data_dir + 'save/', 'config.pkl'), 'wb') as f:
        cPickle.dump(args, f)
    with open(os.path.join(args.data_dir + 'save/', 'chars_vocab.pkl'), 'wb') as f:
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter(
            os.path.join(args.data_dir+'logs/', time.strftime("%Y-%m-%d-%H-%M-%S")))
